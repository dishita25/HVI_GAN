...
gan_losses.py
-------------
GAN loss utilities used by the HVI-GAN training loop.
...
Two independent discriminator/generator loss instances are created — one
for the RGB colour space and one for the HVI colour space — and combined
inside `train_gan.py`.
"""
...
class LSGANDiscriminatorLoss(nn.Module):
    ...
    Args:
        loss_weight (float): Scalar applied to the total discriminator loss.
                             Typically 0.5–1.0.
    """
    ...
    def forward(
        ...
        Args:
            ...
            pred_fake: Discriminator output on fake (generated) images.
                       Must be detached from the generator graph before
                       being passed here.
        Returns:
...
class LSGANGeneratorLoss(nn.Module):
    ...
    Args:
        loss_weight (float): Weight applied to this adversarial term before
                             adding it to the full generator loss.
    """
    ...
    def forward(self, pred_fake: torch.Tensor) -> torch.Tensor:
        """
        Args:
            pred_fake: Discriminator output on fake images (B, 1, H', W').
                       Should NOT be detached – gradients flow back to G.
        Returns:
            Scalar generator adversarial loss.
        """
        return self.loss_weight * 0.5 * self.mse(
...
def build_gan_losses(
    d_weight_rgb: float = 1.0,
    d_weight_hvi: float = 1.0,
    g_weight_rgb: float = 1.0,
    g_weight_hvi: float = 1.0,
):
    """
    Instantiate all four GAN loss objects and return them as a dict.

    Typical usage in train_gan.py:
        losses = build_gan_losses()
        D_rgb_loss_fn = losses['D_rgb']
        D_hvi_loss_fn = losses['D_hvi']
        G_rgb_adv_fn  = losses['G_rgb']
        G_hvi_adv_fn  = losses['G_hvi']
    """
    return {
